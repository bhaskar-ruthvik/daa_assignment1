...
export default function JarvisGraph(props){
    ...
    const [start,setStart] = useState(props.step<2 ? new Vector3(0,0,0) :( props.step==2 ? new Vector3(structRef.current.startPoint.x,structRef.current.startPoint.y,0) : new Vector3(structRef.current.convexHull[props.step-3].x,structRef.current.convexHull[props.step-3].y,0)))
    const [end,setEnd] =useState(props.step<2 || props.step == 2+structRef.current.convexHull.length ? new Vector3(0,0,0) : new Vector3(structRef.current.convexHull[props.step-2].x,structRef.current.convexHull[props.step-2].y,0))
    ...
    const filterPoints = (point,index)=>{
        if(props.step>2 && props.step<=2+structRef.current.convexHull.length){
   ...
   useEffect(()=>{
            ...
            useFrame(()=>{
                ...
                if(props.step>=2 && props.step<structRef.current.convexHull.length+2 && angle<=ang){
...
<pointLight position={[0, 0, 20]} intensity={1000} color={'white'}/>
     {pointsRef.current.map((point,index)=>filterPoints(point,index))}
   ...
   <mesh>
     ...
     {props.step >= 3 && props.step<2+structRef.current.convexHull.length && <Line points={[new Vector3(structRef.current.convexHull[props.step-3].x,structRef.current.convexHull[props.step-3].y,0), new Vector3(structRef.current.convexHull[props.step-3].x+20*Math.cos(angle),structRef.current.convexHull[props.step-3].y+20*Math.sin(angle),0)]} lineWidth={5} color={'white'} /> }
      {props.step>=2 && props.step>=2+structRef.current.convexHull.length && structRef.current.convexHull.map((item,index)=><Line points={[new Vector3(item.x,item.y,0),new Vector3(structRef.current.convexHull[(index+1)%structRef.current.convexHull.length].x,structRef.current.convexHull[(index+1)%structRef.current.convexHull.length].y,0)]} lineWidth={5} color={'yellow'}/>)}
